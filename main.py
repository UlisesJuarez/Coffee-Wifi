...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'mkdfmdkgndjwpoi3skf,dfdldp'
Bootstrap(app)
...
class CafeForm(FlaskForm):
    ...
    location = StringField('Cafe location on google maps (url)', validators=[
                           DataRequired(), URL()])
    open = StringField("Opening time (e.g. 7AM)", validators=[DataRequired()])
    close = StringField("Closing time (e.g. 4:30PM)",
